# %% [markdown]
# # Minimum degree, iteration 2m: the quotient graph, with degrees maintained
#
...
def mdm2_show(A, I, C, degrees, title=None, eliminated=None):
    """Print a quotient graph: adjacency, incidence, cliques, in the order the
    structure holds them. Takes the cached degrees and advances no tag: md2 had to
    recompute a degree to print one, and the point of this file is that nobody
    recomputes. A printed degree for an untouched vertex is whatever the last
    refresh that touched it left behind."""
    n = len(A)
...
def mdm2_neighbors(A, I, C, mark, tag, u):
...
def mdm2_eliminate(A, I, C, mark, tag, eliminated, pivot):
    ...
    """
    neighbors, tag = mdm2_neighbors(A, I, C, mark, tag, pivot)
    ...
    #
    # Two tags, and both are about cliques. They differ in which SIDE of a clique
    # they name, which is the quotient graph's two-sided representation showing up
    # in the mark array:
    #
    ...
    #
    # One tag would also work, and it is worth knowing why two are used. The two
    # tags stamp different sides and the two loops query different sides, so they
    # cannot meet: A[u] holds only vertices, I[u] only clique ids. With a single
    # value each test would still name its own set. Two make that true of the tag
    # alone rather than of the lists as well, and the cost of the second is one
    # advance per elimination, the difference between 3 and 2 in the tag-overflow
    # table's eliminate column. Parked until md3, whose third stamped set, the
    # merged vertices, is on the MEMBER side alongside C[pivot] and so is the first
    # case where two tags would share a side.
    tag += 1
...
def mdm2_minimum_degree(G):
    """md2's algorithm with the degrees cached and refreshed rather than recomputed.

    Three phases per iteration where md2 has two. The picker reads the array; the
    eliminator forms C[pivot], which is the one union nobody can avoid; and then the
    refresh recomputes a degree for each member of C[pivot] and nobody else."""
    n = len(G)
    ...
    # No clique yet, so a vertex's reachable set is exactly its adjacency and the
    ...
    for iteration in range(n):
        ...
        # pivot_clique_tag and absorbed_cliques_tag live across the whole prune loop. Never
        # observed to fire.
        if tag > TAG_CEILING:
            mark = [-1] * n
            tag = 0
