// mda2.cpp -- minimum degree, step 2a: the quotient graph, with the degree bounded.
//
...
// every step, touched or not. What makes the tighter form available is MAINTAINED
...
std::vector<std::int32_t> mda2MinimumDegree(const Graph& G) {
    ...
    for (std::int32_t step = 0; step < static_cast<std::int32_t>(n); ++step) {
        ...
        title << "step " << step << ": eliminate " << pivot << " (bound " << best
              << ", degree " << degree
