...
std::vector<std::int32_t> md4Neighbors(const AdjacencyGraph& A, const IncidenceGraph& I, const Cliques& C,
...
void md4Show(const AdjacencyGraph& A, const IncidenceGraph& I, const Cliques& C,
...
void md4ShowState(const std::vector<std::uint32_t>& degrees,
...
std::size_t md4Storage(const AdjacencyGraph& A, const IncidenceGraph& I, const Cliques& C) {
...
md4Eliminate(AdjacencyGraph& A, IncidenceGraph& I, Cliques& C, std::vector<bool>& eliminated,
...
std::vector<std::int32_t> md4MinimumDegree(const AdjacencyGraph& G) {
    ...
    while (numEliminatedVertices < n) {
        ...
        for (std::int32_t u = 0; u < static_cast<std::int32_t>(n); ++u) {
            ...
            if (pivot == NIL || degrees[u] < minDegree) { pivot = u; minDegree = degrees[u]; }
        }
        ...
        // inside md4Eliminate, which holds three stamps live in turn: pivotCliqueTag and
        // absorbedCliquesTag across the prune loop, then the merged set across the C[pivot]
