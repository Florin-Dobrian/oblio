// mdm2.cpp -- minimum degree, iteration 2m: the quotient graph, with degrees maintained.
//
...
//
// Build:  g++ -std=c++17 -O3 mdm2.cpp -o mdm2_cpp  (or: make)
// Run:    ./mdm2_cpp
//         ./mdm2_cpp 3      just the third example
...
std::vector<std::int32_t> mdm2MinimumDegree(const AdjacencyGraph& G) {
    ...
    for (std::uint32_t iteration = 0; iteration < n; ++iteration) {
        ...
        for (std::int32_t u = 0; u < static_cast<std::int32_t>(n); ++u) {
            ...
            if (pivot == NIL || degrees[u] < degrees[pivot]) pivot = u;
        }
        ...
        auto [neighbors, absorbedCliques, prunedEdges] =
            mdm2Eliminate(A, I, C, eliminated, mark, tag, pivot);
        ++numEliminations;
...
int main(int argc, char** argv) {
    ...
    if (argc > 2 && std::string(argv[1]) == "grid") {
        ...
        mdm2MinimumDegree(gridGraph(side));
        return 0;
    }
    ...
    // All of them by default. To run just one, pass its number: ./mdm2_cpp 3
