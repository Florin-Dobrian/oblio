...
//
// The bound, and this is the tighter of the two the ladder uses. Term by term, as
// mdam2RefreshBounds computes them:
//
//     bound(u) = |A[u] - C[pivot]|                    -> A[u].size()
//              + |C[pivot] - {u}|                     -> C[pivot].size() - 1
//              + sum over c in I[u], c != pivot,
//                           of |C[c] - C[pivot]|      -> sum of outside[c]
//
// with degree(u) <= bound(u). Two terms need no work: the eliminator has already
// pruned C[pivot] out of A[u], and u is a member of C[pivot].
//
// Why this form is available here and not in mda2. It is stated against C[pivot], so
// it applies only to a vertex IN C[pivot]. mda2 recomputes, so it must bound every
// live vertex including untouched ones, and falls back on the PIVOT-FREE form:
//
//     PIVOT-FREE            sum over c in I[u]              of ( |C[c]| - 1 )
//     AGAINST C[pivot]      sum over c in I[u], c != pivot, of |C[c] - C[pivot]|
//
// which is strictly looser, since u is one member of C[pivot]:
//
//     |C[c] - C[pivot]|   <=   |C[c] - {u}|   =   |C[c]| - 1
//
// So maintenance buys the tighter bound as well as the smaller refresh set, and this
// file is the earliest in the ladder that computes what amd1 computes.
//
...
//
// Why refreshing only the new clique is enough: eliminating a pivot can only change
// the degrees of the vertices it REACHED. Pruning and clique membership change only
// for members of the new clique; absorption deletes cliques the pivot belonged to,
// and every member of such a clique is reachable from the pivot, hence in the new
// clique. Nothing else in the graph can tell that an elimination happened.
//
// What maintenance buys, and it is two things, both from the refresh set narrowing
// from every live vertex to the members of C[pivot]:
//
...
//   - A TIGHTER BOUND BECOMES EXPRESSIBLE, because every vertex worked on is now a
//     member of C[pivot]. This file does not use it: it computes exact degrees.
//     That goody is taken in mdam2.
//
// What does not change is the SCAN: the picker still walks all n slots and skips
// the eliminated, so it is O(n) here as in md2; what falls is the WORK, from a
// union per live vertex to a union per member of C[pivot]. Removing the walk itself
// is a third idea, degree buckets, which is md5.
//
// And the work moves in time as well as in size. md2 computes degrees in the
// picker, BEFORE the elimination; here they are computed in a refresh phase AFTER
// it, so a printed degree for an untouched vertex is a cached value from whenever
// it was last refreshed. See the README section "Zooming in on md2".
//
...
// where the bound is loose the picker may prefer a vertex the exact degree would not
// have chosen.
//
// Build:  g++ -std=c++17 -O3 mdam2.cpp -o mdam2_cpp  (or: make)
// Run:    ./mdam2_cpp
...
std::vector<std::int32_t> mdam2Neighbors(const Graph& A, const Graph& I, const Cliques& C,
...
// The bound is read from the array, since nobody recomputes it; the degree beside it
// IS recomputed, which costs the union the bound exists to avoid, and is here to be
// looked at.
void mdam2Show(const Graph& A, const Graph& I, const Cliques& C,
...
//
// The third term is one number per clique, shared by the whole group, and is obtained
// by SUBTRACTION rather than by intersecting anything. Start each touched clique at
...
void mdam2RefreshBounds(const Graph& A, const Graph& I, const Cliques& C,
    ...
    ++tag;
    const std::int32_t touched = tag;
    for (std::int32_t u : clique)                 // initialize once per clique
        for (std::int32_t c : I[u]) {
            if (c == pivot) continue;
            if (mark[c] != touched) { mark[c] = touched; outside[c] = C.at(c).size(); }
        }
    for (std::int32_t u : clique)                 // subtract the overlap with C[pivot]
        for (std::int32_t c : I[u])
            if (c != pivot) --outside[c];
    const std::size_t cliqueTerm = C.at(pivot).size() - 1;   // |C[pivot] - {u}|
    for (std::int32_t u : clique) {
        std::size_t bound = A[u].size() + cliqueTerm;        // A[u] already pruned
        for (std::int32_t c : I[u])
            if (c != pivot) bound += outside[c];
        bounds[u] = bound;
    }
}
...
std::size_t mdam2Storage(const Graph& A, const Graph& I, const Cliques& C) {
    std::size_t total = 0;
...
mdam2Eliminate(Graph& A, Graph& I, Cliques& C, std::vector<bool>& eliminated,
             std::vector<std::int32_t>& mark, std::int32_t& tag, std::int32_t pivot) {
    const std::vector<std::int32_t> neighbors = mdam2Neighbors(A, I, C, mark, tag, pivot);
...
std::vector<std::int32_t> mdam2MinimumDegree(const Graph& G) {
    ...
    for (std::int32_t step = 0; step < static_cast<std::int32_t>(n); ++step) {
        ...
        if (picked > mdam2Neighbors(A, I, C, mark, tag, pivot).size()) ++loosePicks;
        ...
        mdam2Show(A, I, C, bounds, mark, tag, title.str(), &eliminated);
    }
...
int main(int argc, char** argv) {
    ...
    // All of them by default. To run just one, pass its number: ./mdam2_cpp 3
