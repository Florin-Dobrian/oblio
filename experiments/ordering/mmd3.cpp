...
std::vector<std::int32_t> mmd3MinimumDegree(const AdjacencyGraph& G, std::int32_t delta = 0) {
    ...
    // WE FILE AT THE DEGREE, HERE AND AT EVERY REFRESH. genmmd does not: mmdint files a
    // degree-0 vertex under 1, `if(dg==0)dg=1`, and mmdupd files every vertex under its
    // degree PLUS ONE. Two scales in one bucket array, so a refreshed vertex is penalised
    // by exactly one against one no pivot has reached, and the minimum is not always the
    // minimum. private/MmdCorrected.cpp is genmmd with that repaired and is what this
    // prototype and production both match; private/MmdVendored.cpp keeps the original.
    //
    // THE FLOOR USED TO BE HERE, and this comment used to say it existed to match the
    // vendored output. The measurement it carried still holds and is now the record of what
    // dropping it does: same prepass set and identical nnz(L) on all 300 random graphs
    // tried, a different permutation on 212 of them. The prepass below takes bucket 0 and
    // then bucket 1.
    //
