...
// MmdChained.h - MmdFlat on THE MMD ORACLE'S DEAD-SEGMENT CLIQUE STORAGE. PERMANENT, and for two
// reasons. It is the ALIGNMENT VEHICLE for a differential against that oracle, holding cliques the
// way it does so that whatever still differs is either layout or an improvement to carry back into
// our own ladder; and it is the PREDICTABLE-SPACE version of MmdFlat, staying inside `O(n + m)` so
// the answer is reachable whenever the input fits, which our arena cannot promise. It costs 4 to
// 10 percent. See src/MmdChained.cpp.
//
// It returns MmdFlat's permutation, which is the mmd oracle's, so it is an oracle for itself.
//
