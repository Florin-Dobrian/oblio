...
first is the honest baseline, **hand-written flat**: it walks `colPtr` raw through friendship,
calling no accessor, as one would if CSC were the only storage. If the templated version matches it,
reaching a column through the accessor costs nothing.
