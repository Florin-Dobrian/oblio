            ...
            for x in range(len(group)):
                ...
                for y in range(x + 1, len(group)):
                    ...
                    if same and size_u == size_v:
                        ...
                        #
                        # u's REACHABLE SET is unchanged by the merge, and that half
                        # is worth keeping: the two were adjacent to each other, v
                        # leaves the graph, and an external degree excludes u's own
                        # supervariable, so what u can reach is exactly what it was.
                        #
                        # And the degree IS recomputed, by one subtraction. The
                        # bound a few lines above was written with u's weight as it
                        # stood BEFORE this merge, and the weight appears inside the
                        # bound, in `degme - weight(u)` and in the `num_left -
                        # weight(u)` cap. So absorbing v leaves the filed value one
                        # bucket too high per original vertex taken, and u is never
                        # picked as early as its size has earned.
                        #
                        # This comment used to say the opposite, that nothing was
                        # stale because an external degree excludes u's own
                        # supervariable and only the WEIGHT changes. The first half
                        # is true and the second is the whole difficulty: the buckets
                        # are keyed on a degree that has the weight subtracted in it.
                        # AMD_2 has no such problem because it subtracts `nvi` in the
                        # pass that restores the degree lists, which runs AFTER
                        # supervariable detection and so reads the post-merge weight.
                        # Found by aligning amd3 against it, where the same timing is
                        # ledger entry 4.
                        #
                        # One subtraction rather than a recomputation, because all
                        # three terms of the bound's minimum shift by the same amount
                        # when the weight grows, so the minimum shifts with them. It
                        # remains an upper bound for the same reason: the true
                        # external degree drops by exactly weight(v) as v stops being
                        # outside u's supervariable and becomes part of it.
                        weight_v = len(super_members[v])
                        super_members[u] += super_members[v]
                        amd2_refile(buckets, filed, degrees, u, degrees[u] - weight_v)
                        super_members[v] = []
