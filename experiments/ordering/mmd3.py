...
# about 7. mmd1 cannot have it, having no two-source path and no live merges.
#
# THE AMD LAYERS COULD, and this paragraph used to say they could not. It argued
# that AMD files at an external degree, which excludes a vertex's own supervariable
# and so does not move when its weight changes. The external degree does not move;
# the `- weight(u)` term inside the amd bound does, and it is the term that decides
# the bucket. amd2, Amd2 and Amd2B carried exactly this defect and it was costing 3
# to 9 percent of fill on grids, found on 2026-08-08 by aligning amd3 against the
# vendored AMD, where it is that ledger's entry 4, and fixed the same day. amd1 and
# Amd1B genuinely cannot have it, having no live merges at all.
#
# Worth keeping as a lesson about ledgers rather than only as a correction. The
# claim was reasoned from a true premise and never checked against the quantity
# actually filed. A defect ruled out by an argument is ruled out only as far as the
# argument reaches, and the same sentence stood in the README for a month.
#
...
def mmd3_minimum_degree(G, delta=0):
    ...
    while num_eliminated_vertices < n:
        ...
        for element in reversed(batch):
            ...
            # mmdupd's q2h chain, head-pushed there. See mmd3_neighbors.
            ...
            # mmdupd's qxh chain, the same shape. See mmd3_neighbors.
            for u in reversed(many_source_queue):
