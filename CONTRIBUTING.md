...
> build/test/style so it doesn't fork from the notes below.

For now, the working notes are canonical; this file only points at them:

- **Build & test**, see CLAUDE.md (`## Build`, `## Tooling`).
- **Coding rules**, invariants in CLAUDE.md; conventions in CODING_RULES.md;
  mechanical style enforced by `.clang-format` / `.clang-tidy`.
- **Design rationale & history**, notes/DESIGN_DECISIONS.md, and archive/ for the
  development history.
- **Porting workflow**, one unit at a time, verified against Oblio 0.9; status
  tracked in notes/PORTING_LEDGER.md.
