...
int main(){
    ...
    { auto A=tridiagFull(30); reqSym(A,"tridiag n=30        : symmetric");
      OrderEngine ord(Ordering::MMD2); Permutation P; ord.compute(A,P);
      ElmForest f; bool ok=eng.compute(A,P,f); std::size_t roots=0; bool inv=validEtree(f.parent(),roots);
      ck(ok && inv && f.size()==30 && roots>=1, "tridiag n=30 + MMD2 : valid etree"); }
    ...
    { // ---------------------------------------------------------------------------------------
      ...
      Permutation P;
      OrderEngine oe; oe.setOrdering(Ordering::MMD2);
      oe.compute(A, P);
