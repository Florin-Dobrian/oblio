...
std::vector<std::int32_t> md3Neighbors(const AdjacencyGraph& A, const IncidenceGraph& I, const Cliques& C,
...
void md3Show(const AdjacencyGraph& A, const IncidenceGraph& I, const Cliques& C,
             std::vector<std::int32_t>& mark, std::int32_t& tag,
...
std::size_t md3Storage(const AdjacencyGraph& A, const IncidenceGraph& I, const Cliques& C) {
...
md3Eliminate(AdjacencyGraph& A, IncidenceGraph& I, Cliques& C, std::vector<bool>& eliminated,
...
std::vector<std::int32_t> md3MinimumDegree(const AdjacencyGraph& G) {
...
int main(int argc, char** argv) {
    ...
    if (argc > 2 && std::string(argv[1]) == "grid") {
        ...
        md3MinimumDegree(gridGraph(side));
        return 0;
    }
