...
// MAX_IDX caps a SIZE rather than naming an index; construction guards check inputs
// against it so an over-range value fails loudly rather than wrapping at a narrowing cast.
...
// The largest admissible SIZE, as a std::size_t for comparison against sizes and offsets.
// Equal to INT32_MAX. Bounds both the matrix dimension and, for now, nnz (the entry count is
// narrowed to int at the vendored AMD/MMD boundary); those two caps happen to coincide at this
// value today but have different origins.
//
// SO IT IS ONE ABOVE THE LARGEST INDEX. A guard rejects `size > MAX_IDX`, so n reaches MAX_IDX
